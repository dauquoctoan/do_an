...
const EditorComponent = (props: IProps) => {
  ...
  const uploadImageCallBack = async (file: any) => {
    ...
    const res: any = await ApiClient.uploadFile(
      'Upload/UploadImage',
      fmData,
      config
    )
  ...
  const onEditorStateChange = (editorState: any) => {
    ...
    const isContainOnlySpaces =
      !isEditorEmpty && !currentPlainText.trim().length
    if (isContainOnlySpaces || isEditorEmpty) {
      props.setIsAllSpace(true)
    } else {
      props.setIsAllSpace(false)
    }
  }
