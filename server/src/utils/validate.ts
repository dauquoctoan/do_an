...
const joi: Joi.Root = Joi.defaults((schema: Joi.Schema) => {
    return schema.options({
        abortEarly: false,
        allowUnknown: true,
        messages: {
            'string.min': '{#label} không được ít hơn {#limit} ký tự',
            'string.max': '{#label} không được lớn hơn {#limit} ký tự',
            'string.regex.base': '{#label} không hợp lệ',
            'string.email': '{#label} là email',
            'string.required': '{#label} là bắt buộc',

            'any.unknown': '{#label} không hợp lệ',
            'any.invalid': '{#label} chứa giá trị không hợp lệ',
            'any.empty': '{#label} không được để trống',
            'any.required': '{#label} là bắt buộc',
            'any.only': '{#label} phải có giá trị {{#valids}}',
            'any.default':
            ...
            'number.ref':
                '{#label} references "{{#ref}}" which is not a number',
...
const errors = {
    ...
    string: {
        ...
        min: 'length can phai must be at least {{limit}} characters long',
